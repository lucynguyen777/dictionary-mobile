...
function tokenizeReaderText(text: string) {
  ...
  // Use Intl.Segmenter if available for CJK/Burmese/Tibetan or general advanced segmentation
  if (typeof Intl !== 'undefined' && 'Segmenter' in Intl) {
    try {
      const hasCjkOrBurmeseOrTibetan = /[\u4e00-\u9fa5\u3040-\u30ff\u31f0-\u31ff\uac00-\ud7af\u1000-\u109F\u0F00-\u0FFF]/.test(text);
      if (hasCjkOrBurmeseOrTibetan) {
        let locale = 'zh';
        if (/[\u1000-\u109F]/.test(text)) {
          locale = 'my';
        } else if (/[\u0F00-\u0FFF]/.test(text)) {
  ...
  // Fallback regex supporting CJK/Burmese characters individually, Tibetan syllables (ending with optional tsheg \u0F0B), and other alphabetic/abjad scripts grouped
