...
export async function openExpoUserDatabase(): Promise<UserSqliteDatabase> {
  throw new Error('User SQLite runtime is disabled on Expo web; using AsyncStorage fallback.');
}

export async function deleteExpoUserDatabase() {
  throw new Error('User SQLite runtime is disabled on Expo web; using AsyncStorage fallback.');
}
