...
This app is ready to ship as v1.2.2 with the completed feature set documented in `docs/product-progress.md`.
...
- Confirm App Privacy/Data Safety answers match v1.2.2: local-first storage, local export/reset, Supabase Auth/manual beta Cloud Sync only when configured, AI/DeepL through backend proxy only when configured, and no Google OAuth in this release.
