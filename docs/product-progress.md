...
- [~] IN PROGRESS [HARD]: Offline dictionary MVP Phase 1: SQLite schema and pack pipeline documented in `docs/offline-dictionary-mvp.md`, prototype JSONL-to-gzip pack builder added in `scripts/build-offline-pack.mjs`, and profile acknowledgement copy notes Wiktionary/Kaikki CC BY-SA/GFDL attribution. Runtime SQLite import, download management, and per-entry offline lookup are still pending.
