...
**v1.3.7 - Supabase Auth And Cloud Sync Production Smoke** - IN PROGRESS
...
- [~] IN PROGRESS [HARD]: Supabase project health and Expo public env configuration are verified locally and in Vercel Production/Development; redirect allow-list, callback, and Preview-branch smoke remain to be verified.
- [x] DONE [HARD]: Added/applied authenticated table grants and passed live own-row/cross-user RLS probes: own insert/select works, cross-user read/update/delete returns zero rows, and anonymous access is blocked.
...
**Module: Kannada Native Source And Inflected-Relations Readiness** - DONE WITH SECTION-FILTER GATE
- Module Completion Plan: validate Kannada Wiktionary as a mixed-language native-edition extraction candidate and reuse conservative suffix fallback for related words without broad glide/sandhi guessing.
- Acceptance criteria: Kannada-section filtering risk is recorded; exact lookup stays first; NFC-safe inflected relations resolve for verified fixtures; production promotion waits for measured corpus/suffix/tokenization/offline gates.
- [x] DONE [HARD]: Audited current Kannada adapter, native-edition source risk, morphology, and stale plan status.
- [x] DONE [MEDIUM]: Made Kannada morphology input NFC-safe and added morphology-aware related-word lookup.
- [x] DONE [MEDIUM]: Documented Kannada extraction/measurement boundaries and reconciled Kannada plan/roadmap truth.
- [x] DONE [MEDIUM]: Added focused source/inflected-relations readiness tests without changing production status.
- [x] DONE [EASY]: Ran verification/security/license audit, then prepared the Kannada source/relations readiness module for commit/push without deploy.
...
- [x] DONE [HARD]: Audited hosted WiktAPI, English-Wiktionary-derived Kaikki Hungarian, native `hu.wiktionary.org`, and morphology-helper paths.
- [x] DONE [MEDIUM]: Added Hungarian NFC/locale-safe morphology input and conservative fixture-backed plural-plus-case/instrumental behavior.
- [x] DONE [MEDIUM]: Documented the native extraction/measurement decision and updated Hungarian plan/roadmap truth.
- [x] DONE [MEDIUM]: Added focused source/case-chain readiness tests without changing production status.
- [x] DONE [EASY]: Ran verification/security/license audit and prepared the Hungarian source/case-chain readiness module for commit/push without deploy.
...
**Module: v1.3.7 Supabase Auth And Cloud Sync Production Smoke** - IN PROGRESS
...
- [x] DONE [HARD]: Re-verified the existing Supabase connection: project health responds successfully, ignored local Expo env is configured, Vercel Production/Development env is configured, and focused auth/sync guard tests pass.
- [x] DONE [HARD]: Applied migration `007_authenticated_table_grants.sql` and passed live cross-user RLS probes with two disposable users.
...
- Acceptance criteria: `DESIGN.md` documents Hick/Fitts/familiarity/proximity/similarity/progressive-disclosure/content-first rules plus the 4px spacing/radius/shadow/button/motion laws; `DesignSystem` gains additive semantic surface, shadow, interaction, motion, and reduced-motion tokens while legacy exports remain; Home and Word no longer show hard-coded light cards/text in dark mode; shared primitives support hover/press/loading/entrance effects with reduced-motion fallback; large surfaces in Profile/Library/Advanced/Reader use tokenized shells where touched; CodeGraph status/affected output is recorded; static/unit/build/UI smoke verification passes before commit/push/deploy.
- [x] DONE [MEDIUM]: Audited and rewrote `DESIGN.md` plus `docs/product-progress.md` for the v1.3.2 design-system module.
- [x] DONE [MEDIUM]: Added semantic design tokens and reusable app primitives for surfaces, buttons, press/hover motion, and loading skeletons.
- [x] DONE [HARD]: Cleaned up dark-mode surface colors on Home, Word, and the highest-risk shells in Profile/Library/Advanced/Reader without removing handlers or routes.
- [x] DONE [MEDIUM]: Added expressive futuristic microinteractions and reduced-motion fallbacks using existing dependencies only.
- [x] DONE [MEDIUM]: Ran verification and CodeGraph locally: `git diff --check`, `npx tsc --noEmit`, `npm run lint -- --max-warnings=0`, `npm test -- --run`, and `npm run build:web:clean` passed; CodeGraph status reported 178 files, 3,331 nodes, 7,397 edges, and affected reported no direct test files for the touched UI/theme files. Security audit found only documented env var names, no committed secret values.
