...
Status: **IN PROGRESS - Auth and cross-user RLS pass; two-session sync smoke remains**.
...
| Remote migrations | Verified | `supabase db push --dry-run` reports the remote database is up to date after migration `007`. |
| Cross-user RLS | Passed | Two disposable users can access their own rows; cross-user read/update/delete return zero rows; anonymous access returns `401`. |
...
- Added and applied `007_authenticated_table_grants.sql` so authenticated users can reach RLS policies without granting table access to `anon`.
- Live cross-user probe passed for own insert/select, blocked cross-user read/update/delete, and blocked anonymous access.
...
3. Add the public Expo variables to any Vercel Preview branch that will be used for smoke.
4. Do not provide or commit a Supabase service-role key to the mobile/web app.
5. Prepare two browsers, devices, or dev-client sessions signed in as the same user for the two-device sync smoke.

## Smoke To Run After Setup Exists

1. Auth smoke from `docs/supabase-auth-manual-smoke.md`.
2. Two-device manual sync from `docs/supabase-cloud-sync-manual-smoke-execution.md`.
3. Regression checks:
...
Do not mark v1.3.7 complete yet. Supabase Auth and cross-user RLS are verified, but keep Cloud Sync at manual beta until redirect/callback and two-session sync pass/fail results are recorded without secrets.
